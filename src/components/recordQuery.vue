<template>
  <div class="warp">
   ...
   <Searcher></Searcher>
   <!-- <div class="post-card">
   <button  @click="deleteAllfocus" class="mp-description-bookingbtn ">全部删除</button >   
   </div>   -->
    <div class="show-post">
      <div class="container bd-box">
        <div v-if="jobList.length>0"  class="tab-content">
          <div v-for="(item) in jobList" :key="item.scenic_id" class="post-card">
            <div class="post-info">
              <div class="post-head">
            ...
            <div class="company-info">
              <div class="company-name">
                <a href="#">{{item.scenic_level}}</a>  <button  @click="deletefocus(item.scenic_id)" class="mp-description-bookingbtn">删除</button >    
              </div>
...
export default {
  ...
  methods:{
    getRecordList(){
      let that = this;
       this.axios({
          url: that.API.BROWSERECORD.GETBROWSERECORD,
          methods: "GET",
          params: {
    ...
    deletefocus(val){
        let that = this;
         return this.axios({
        url:this.API.BROWSERECORD.DELBROWSERECORD,
        params: {
            record_id:val
          }
      }).then(res => {
    ...
    deleteAllfocus(){
        let that = this;
         return this.axios({
        url:this.API.BROWSERECORD.DELALLBROWSERECORD,
        params: {
          }
      }).then(res => {
          console.log(res)
